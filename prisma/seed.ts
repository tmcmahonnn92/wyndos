...
const prisma = isSqliteUrl(databaseUrl)
  ? new PrismaClient({
      adapter: new PrismaBetterSqlite3({ url: resolveSqlitePath(databaseUrl) }),
    } as ConstructorParameters<typeof PrismaClient>[0])
  : new PrismaClient({} as ConstructorParameters<typeof PrismaClient>[0]);

const SEED_TENANT_SLUG = "seed-demo";
const SEED_TENANT_NAME = "Seed Demo Window Cleaning";
...
async function main() {
  ...
  await prisma.tenantSettings.upsert({
    where: { tenantId: seedTenant.id },
    update: { businessName: SEED_TENANT_NAME },
    create: {
      tenantId: seedTenant.id,
      businessName: SEED_TENANT_NAME,
    },
  });

  // Clean only the seed tenant's business data so other tenants remain intact.
  await prisma.customerTag.deleteMany({ where: { customer: { tenantId: seedTenant.id } } });
  await prisma.payment.deleteMany({ where: { tenantId: seedTenant.id } });
  await prisma.job.deleteMany({ where: { tenantId: seedTenant.id } });
  await prisma.workDay.deleteMany({ where: { tenantId: seedTenant.id } });
  await prisma.customer.deleteMany({ where: { tenantId: seedTenant.id } });
  await prisma.area.deleteMany({ where: { tenantId: seedTenant.id } });
  await prisma.holiday.deleteMany({ where: { tenantId: seedTenant.id } });
  await prisma.tag.deleteMany({ where: { tenantId: seedTenant.id } });
  ...
  const [
    ...
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Langold & Tuxford - 4 Weekly",       color: "#3B82F6", sortOrder:  1, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-16")) } }),
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Creswell Model Way - 4 Weekly",      color: "#10B981", sortOrder:  2, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-17")) } }),
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Rogers & Portland - 4 Weekly",       color: "#F59E0B", sortOrder:  3, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-18")) } }),
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Creswell / West St - 4 Weekly",      color: "#EF4444", sortOrder:  4, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-19")) } }),
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Elmton Road - 4 Weekly",             color: "#8B5CF6", sortOrder:  5, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-20")) } }),
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Bolsover & Clowne - 4 Weekly",       color: "#06B6D4", sortOrder:  6, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-23")) } }),
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Edwinstowe - 4 Weekly",              color: "#F97316", sortOrder:  7, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-24")) } }),
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Welbeck & Norton - 4 Weekly",        color: "#6366F1", sortOrder:  8, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-25")) } }),
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Carlton Rd & Rhodesia - 4 Weekly",   color: "#D946EF", sortOrder:  9, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-26")) } }),
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Gateford / Hemmingfield - 4 Weekly", color: "#14B8A6", sortOrder: 10, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-27")) } }),
    prisma.area.create({ data: { tenantId: seedTenant.id, name: "Dale Close - 4 Weekly",              color: "#84CC16", sortOrder: 11, scheduleType: "WEEKLY", frequencyWeeks: 4, nextDueDate: utcDay(new Date("2026-03-30")) } }),
  ]);
  ...
  await seedCustomers(seedTenant.id, langold.id,          LANGOLD_CUSTOMERS);
  await seedCustomers(seedTenant.id, modelWay.id,         MODEL_WAY_CUSTOMERS);
  await seedCustomers(seedTenant.id, rogersPortland.id,   ROGERS_PORTLAND_CUSTOMERS);
  await seedCustomers(seedTenant.id, creswellWestSt.id,   CRESWELL_CUSTOMERS);
  await seedCustomers(seedTenant.id, elmtonRd.id,         ELMTON_CUSTOMERS);
  await seedCustomers(seedTenant.id, bolsover.id,         BOLSOVER_CUSTOMERS);
  await seedCustomers(seedTenant.id, edwinstowe.id,       EDWINSTOWE_CUSTOMERS);
  await seedCustomers(seedTenant.id, welbeck.id,          WELBECK_CUSTOMERS);
  await seedCustomers(seedTenant.id, carltonRhodesia.id,  CARLTON_RHODESIA_CUSTOMERS);
  await seedCustomers(seedTenant.id, gateford.id,         GATEFORD_CUSTOMERS);
  await seedCustomers(seedTenant.id, daleClose.id,        DALE_CLOSE_CUSTOMERS);

  const total = await prisma.customer.count({ where: { tenantId: seedTenant.id } });
  console.log(`✅ Seeded ${total} customers across 11 areas`);
  console.log(`   Tenant: ${SEED_TENANT_NAME} (${SEED_TENANT_SLUG})`);
  console.log("\n🎉 Seed complete!");
